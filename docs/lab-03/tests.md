...
Covers role guards on every route that exists after this issue, plus the client-side consequence of gating them. Several rows planned here need routes or screens (queue, admin, staff ticket detail, Internal Notes, `/tickets/mine`, a real router) that do not exist until their own issue lands; those stay `Planned` with a note on where they move. Ownership rewiring to the session identity (BR-03) and removing the Lab 2 selector (FR-19) are the Requester Regression issue's job, not this one's — the routes below keep their Lab 2 `:requesterId` path/body parameter for now, gated by role only.

| Test ID | Type | Requirement / AC | What It Tests | Expected Result | Automated Test File | Final |
| --- | --- | --- | --- | --- | --- | --- |
| UNIT-06 | Unit | BR-08, AC-16, AC-17 | Guard check order | Folded into SEC-09 below (real routes through the real app give higher-fidelity coverage than mocking the middleware in isolation): session `401` beats a malformed id, the password gate `403` beats it too, and role `403` beats it; only a permitted, gated caller reaches the handler's own `400`. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| API-08 | API | AC-04, BR-04, BR-09 | Requester requests Internal Notes | Moves to the IT Staff Ticket Operations issue — `/internal-notes` does not exist until then. | `server/tests/lab-03/comments-notes.api.test.ts` | Planned |
| SEC-03 | Security | AC-16, FR-17 | Unauthenticated access matrix | Every route protected after this issue (`api-spec.md` §16 entries that exist today) is called without a cookie. Each returns `401 UNAUTHENTICATED` with no data and a cookie-clearing header. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-04 | Security | AC-02, BR-02 | Password-change gate matrix | A Requester with `mustChangePassword` calls every currently protected route. All return `403 PASSWORD_CHANGE_REQUIRED` — including the Requester-only ones this account's role would otherwise pass, proving the gate runs before the role check. `/auth/me`, `/auth/logout`, and `/auth/change-password` remain exempt. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-05 | Security | AC-17, BR-05, BR-07 | Requester denied staff and admin operations | Moves to the IT Staff and Administrator issues — queue, claim/owner/IT-Priority/status, assignable users, and `/api/admin/...` do not exist until then. | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-06 | Security | AC-17, AC-55, BR-49 | IT Staff denied admin endpoints | Moves to the Administrator user management issue — `/api/admin/users...` does not exist until then. | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-07 | Security | AC-20, AC-45, BR-05, BR-07, BR-25 | Role-restricted routes, both directions | IT Staff and Administrators get `403 FORBIDDEN` on every Requester-only route (create Ticket, My Tickets, Ticket Detail, Attachment list/upload/download/removal, the Development Requester list) with the database unchanged; every role can reach the any-role routes (Categories, Related Systems). | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-08 | Security | AC-21, BR-09 | Cross-Requester access without leaks | Moves to the Requester Regression issue — today's cross-Requester response is still Lab 2's `403` (ownership still keyed on the URL, not the session); the `404` no-leak behavior needs BR-03's rewiring first. | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-09 | Security | BR-08 | Check-order leaks | Against a malformed id: unauthenticated → `401`; a gated-but-blocked Requester → `403 PASSWORD_CHANGE_REQUIRED`; the wrong role (IT Staff) → `403 FORBIDDEN`; the right role → `400` from the handler's own validation, last. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-10 | Security | AC-03, BR-03 | Client-supplied `requesterId` ignored | Moves to the Requester Regression issue, which does the rewiring this proves. | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-11 | Security | AC-19, FR-19 | Removed Lab 2 routes | Moves to the Requester Regression issue, which removes them. Today `GET /api/development-requesters` still exists, now gated to the Requester role (covered by SEC-03/SEC-07 above). | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-12 | Security | BR-22, AC-50 | Role read per request | An IT Staff session is denied a Requester-only route; with no admin endpoint yet to change a role through the API, the same underlying mechanism is proven directly — the role is updated in the database, and the very next request on the *same* session (no new login) reflects it. Repeated with a real admin endpoint once one exists. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-13 | Security | AC-25, BR-26 | Requester responses exclude internal data | Moves to the IT Staff Ticket Operations issue — `itPriority`, `allowedStatusTransitions`, and Internal Notes do not exist on any response yet. | `server/tests/lab-03/authorization.api.test.ts` | Planned |
| SEC-14 | Security | AC-59, BR-60 | Safe errors from the middleware itself | With `prisma.session.findUnique` mocked to throw, a request with an otherwise-valid cookie gets a generic `500 INTERNAL_ERROR` with no stack, SQL, path, or Prisma text. (Per-route safe-500s were already covered by the migrated Lab 2 suites; this is the new middleware's own failure path.) | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| SEC-15 | Security | BR-17 | CORS and cookie CSRF posture | The configured `CLIENT_ORIGIN` gets back that exact `Access-Control-Allow-Origin` with `Access-Control-Allow-Credentials: true`; a foreign `Origin` gets no allow-origin header at all; `*` is never returned; the session cookie is `HttpOnly` and `SameSite=Lax`. | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| UI-09 | UI | AC-18, FR-13 | Shell identity and profile menu | Delivered with the Authentication Foundation issue: the header shows the user's name and role badge, and the profile menu shows name, email, role, Change Password, and Log Out, opening with `aria-expanded` and closing on Escape. | `client/tests/lab-03/AppShell.test.tsx` | Pass |
| UI-10 | UI | AC-18, FR-14 | Role navigation | Partial for now: a signed-in Requester still sees the Requester screens unchanged; IT Staff and Administrators see a placeholder ("Nothing to show yet") inside the normal shell instead of the Requester screens, since gating the API without this would leave those roles looking at a wall of `403`s. Real per-role navigation items (Ticket Queue, User Management) arrive with their own issues. | `client/tests/lab-03/RoleAvailability.test.tsx` | Pass (partial) |
| UI-11 | UI | AC-13 | Logout | Delivered with the Authentication Foundation issue: Log Out shows "Signing out…", and even on an API failure the client clears state and shows Login with "You have signed out." | `client/tests/lab-03/AppShell.test.tsx` | Pass |
| UI-12 | UI | AC-13, AC-16, FR-15 | Unauthenticated guard | Delivered with the Authentication Foundation issue: the Session Check state renders while `/auth/me` is pending, and a failed check offers Retry; a `401`/no session shows Login with no protected content rendered. (There is no client router yet, so there is no `returnTo` to restore — that lands with routing in a later issue.) | `client/tests/lab-03/AppShell.test.tsx` | Pass |
| UI-13 | UI | AC-02 | Password-change guard | Delivered with the Authentication Foundation issue: `mustChangePassword` shows the mandatory Change Password screen with no way around it (no navigation, no Cancel, survives reload), and a `403 PASSWORD_CHANGE_REQUIRED` from the API is what every gated route now returns while it is true (SEC-04 above). | `client/tests/lab-03/ChangePassword.test.tsx`, `server/tests/lab-03/authorization.api.test.ts` | Pass |
| UI-14 | UI | AC-18, AC-55, FR-16 | Forbidden and Not Found | Moves to whichever issue adds the client router and the first role-specific route (Ticket Queue) — there is no `/queue` or `/admin/users` URL yet for a Forbidden panel to guard. `RoleAvailability` (UI-10) is today's placeholder, not this panel. | `client/tests/lab-03/AuthGuard.test.tsx` | Planned |
| UI-15 | UI | AC-14 | Session ended mid-use | Not yet implemented: a `401` from a Lab 2 screen's own `fetch` (e.g. My Tickets) currently shows that screen's generic failure banner, not a redirect to Login with "Your session has ended…". Needs a shared response interceptor, added with the Requester Regression issue when Lab 2's `api.ts` calls are revisited for BR-03 anyway. | `client/tests/lab-03/AuthGuard.test.tsx` | Planned |
| UI-16 | UI | AC-19, FR-19 | Selector and client state removed | Moves to the Requester Regression issue, which deletes the selector. | `client/tests/lab-03/AuthGuard.test.tsx` | Planned |
